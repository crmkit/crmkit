...
// for crmkit binaries. When the binary is built via `go build` without setting
...
// the crmkit repository. Returns the tag name, the release URL, and any error.
...
func FormatUpdateNotice(r *CheckResult) string {
	...
	return fmt.Sprintf(
		"A new version of crmkit is available: %s → %s\nRelease: %s",
